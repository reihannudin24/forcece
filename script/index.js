function init() {
    ...
    const triggerBunnyMessage = (bunny, classToAdd) => {
        bunny.el.setAttribute('message', [
            'Cece rejeki nggak kemana kok! 💰✨',
            'Pasti ada tempat yang cocok! 🏢💕',
            'Proses itu butuh waktu! ⏰🌱',
            'Cece u udah keren banget! 🌟💪',
            'Jangan down ya cece!! 🤗💙',
            'Aku bangga sama usaha cece! 👏❤️',
            'Inget ce yang terbaik akan datang! 🎁🌈',
            'Keep trying, love! 💪💕',
            'Semangat cece ku! 🔥🥰',
            'Gass nanti kita HO lagi ce hehe 🍕🎮'
        ][randomN(5) - 1])
    ...
    new Array(16).fill('').forEach(()=> addBunny())
